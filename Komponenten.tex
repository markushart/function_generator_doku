...
\begin{figure}[h]
  \includesvg[width=14cm]{function_generator}
...
Zusätzlich kann der Zähler asynchron auf Null gesetzt werden, wenn der Eingang \lowactive{R} auf 0 gesetzt wird.
...
Beispielsweise kann der \code{high}-Wert der Funktionen mit dem Befehl \code{0x01000FFF} auf das Maximum der Ausgangsspannung gesetzt werden.
...
Es gibt ein \code{\lowactive{SYNC}} Signal, vergleichbar mit dem \code{\lowactive{SS}} Signal von SPI, ein \code{CLK} Signal und zwei Datenleitungen \code{DINA} und \code{DINB} für Kanal Eins und Zwei.

\subsection{DAC-Kanal} \label{Comp:DAC:Channel}
Ein einzelner Kanal kümmert sich um die Serialisierung des anliegenden zwölf Bit Werts \code{DATA\_in}.
...
Für diese Implementation werden sie allerdings auf \code{low} gehalten, so dass der DAC im Normalbetrieb läuft.
Zusätzlich werden die ersten beiden gesendeten Bits ignoriert, innerhalb einer Übertragung werden also 16 Bit geschickt.
Das Senden ist dann erfolgreich, wenn das \code{\lowactive{SYNC}} Signal 16 DAC-Takte lang auf \code{low} gehalten wurde. Sodann liegt am analogen Ausgang die entsprechende Spannung an.
